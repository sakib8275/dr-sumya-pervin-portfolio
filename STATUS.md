...
| git | **in sync and clean** — `origin/master` and local `master` both at `4460d34` (`git ls-remote` confirmed), working tree clean. Note the deployed code is live regardless of git: `wrangler pages deploy` uploads the working tree, not a git ref | `git ls-remote`, 2026-08-03 22:21 UTC |
