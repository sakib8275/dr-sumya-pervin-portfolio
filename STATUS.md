...
| CI on GitHub | ✅ **GREEN on its first real run** — run `30874041644` on push of `c6da817`: both jobs passed (`npm test` 28 s; **Playwright DOM layer 59 s**, browser download and all, on a bare runner). Red is **proven locally**: renaming `validateSlot` made `npm test` exit **1**; reverting returned exit **0** and 204/204. ⚠️ One annotation, not a failure: `actions/checkout@v4` and `actions/setup-node@v4` target Node 20, which GitHub has deprecated and is force-running on Node 24 — bump both to `@v5` before that becomes an error and silently disables this gate | `gh run view`, 2026-08-04 |
