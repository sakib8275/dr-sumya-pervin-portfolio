...
| **F11 ops** | **DONE.** `.github/workflows/ci.yml` (`npm test` + e2e on push/PR; **never deploys**), `.github/workflows/uptime.yml` (30-min probe of `/api/config/public` shape + homepage + HSTS/CSP — all three dry-run green against production), `npm run backup:d1` + `docs/RUNBOOK-BACKUP.md` (**restore drill proven**: exported 4/4 tables, replayed into a wiped local D1, counts matched), and JSON write logs | see rows below, 2026-08-04 |
| CI goes red | **proven locally**: renaming `validateSlot` made `npm test` exit **1**; reverting returned exit **0** and 204/204. Not yet proven *on GitHub* — the workflows are committed but have never run there | local exit-code check, 2026-08-04 |
...
| git | ⚠️ **local `master` is AHEAD of `origin/master` and not yet pushed.** Compare `git log --oneline -1` against `git ls-remote origin master` — do not paste a bare hash here, this row is deliberately hash-stable. The Phase-2 work (F10, F11, F12) is committed locally only; **pushing needs operator confirmation**, and until it happens the CI and uptime workflows have never run on GitHub. Note the deployed code is live regardless of git: `wrangler pages deploy` uploads the working tree, not a git ref | `git ls-remote`, 2026-08-04 |
