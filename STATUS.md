...
| git | **in sync and clean** — `origin/master` and local `master` both at `30d042e` (`git ls-remote` confirmed after push), working tree clean. Note the deployed code is live regardless of git: `wrangler pages deploy` uploads the working tree, not a git ref. The `30d042e` commit carries the verified UX batch (still **undeployed** on Pages) | `git ls-remote`, 2026-08-04 |
