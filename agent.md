...
| Uptime (authoritative) | `workers/probe` (`dr-sumya-probe`) | Cloudflare-native Worker, cron `*/30 * * * *`. Probes `/api/config/public` from Cloudflare's own network (the vantage the zone's Bot Fight Mode cannot challenge — it 403'd GitHub Actions' Azure IPs on 9/9 runs), asserts the JSON **shape** not a bare 200, and emails the doctor once on the DOWN transition and once on RECOVERY. State in the `uptime_state` single-row D1 table (`migrations/002_uptime_state.sql`). Logs via Workers observability; its output is an email, not a URL. |
| Uptime (third-party canary) | `.github/workflows/uptime.yml` | Demoted: GitHub Actions IPs are Bot-Fight-Mode-challenged, so all three steps treat a `cf-mitigated` edge block as the expected state of that vantage and pass (no owner-inbox spam), while still failing on a dead Function (5xx/timeout/wrong shape), a broken homepage, or lost HSTS/CSP. The probe Worker above is the alert that reaches a human. GitHub's scheduler skews and drops runs; treat as "down for a while", not an SLA monitor. |
